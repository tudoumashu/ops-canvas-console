...
+ [新增] Local Workspace hybrid ecommerce：新增 `opsc ecommerce import-template`、`opsc ecommerce create-run` 和 executor 的 VPS PDD API backend，可把已确认远端电商模板重建为本地 canonical template，创建 pending local run，并把远端 run 状态和关键 artifact 同步回 local workspace。
