...
+ [新增] Local Workspace Phase 14：新增 `opsc ecommerce import-template --local-executable` 和 local-first ecommerce run 创建路径，已确认电商模板可在本地 executor 中执行自动 `anime_ip` 素材匹配、`image_edit`、内置 mockup 底版 fallback、项目相对打包和 `sync_local` marker；已完成真实浏览器 local-first Web smoke，hybrid VPS-backed fallback 保持不变。
