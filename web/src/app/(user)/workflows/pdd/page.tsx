...
const statusMeta: Record<PDDRunStatus, { color: string; label: string }> = {
...
export default function PDDRunsPage() {
    ...
    const rows = useMemo(() => {
        ...
        return (query.data?.items || []).filter((item) => !text || item.runId.toLowerCase().includes(text) || item.recentError?.toLowerCase().includes(text));
    }, [keyword, query.data?.items]);

    const columns: ColumnsType<PDDRunItem> = [
        {
            title: "Run",
            dataIndex: "runId",
            render: (_, item) => (
        ...
        },
        {
            title: "日志",
            width: 90,
            render: (_, item) => <Tag color={item.hasLogs ? "green" : "orange"}>{item.hasLogs ? "完整" : "缺失"}</Tag>,
        },
        {
            title: "更新时间",
            dataIndex: "updatedAt",
    ...
    const startRun = async (templateId: string, inputs: Array<Record<string, unknown>>, productConcurrency?: number, maxRetries?: number) => {
        ...
        setStarting(true);
        try {
            const result = await startPDDWorkflowTemplateRun(templateId, { inputs, productConcurrency, maxRetries }, token);
            message.success(`已启动 run ${result.runId || ""}`);
            setStartOpen(false);
            await query.refetch();
        ...
        return (
            <main className="flex h-full items-center justify-center bg-background px-6 text-foreground">
                <Card className="w-full max-w-md">
                    ...
                    <Typography.Paragraph type="secondary">电商工作流会读取 VPS 上的 run 目录和执行运维动作，需要管理员登录后访问。</Typography.Paragraph>
    ...
    return (
        <main className="h-full overflow-auto bg-background text-foreground">
            <div className="mx-auto flex w-full max-w-7xl flex-col gap-5 px-6 py-8">
                <header className="flex flex-wrap items-end justify-between gap-4 border-b border-stone-200 pb-5 dark:border-stone-800">
                    <div>
                        <Typography.Text type="secondary" className="text-xs">
                            root: {query.data?.root || "/opt/pdd-workflow/runs"}
                        </Typography.Text>
                    ...
                    <Space wrap>
                        ...
                        <Button type="primary" icon={<Play className="size-4" />} loading={starting} onClick={() => setStartOpen(true)}>
                            启动工作流
                        </Button>
                        <Button icon={<Workflow className="size-4" />} href="/workflows/ecommerce/templates">
                            工作流模板
                        </Button>
                        <Button icon={<RefreshCw className="size-4" />} loading={query.isFetching} onClick={() => void query.refetch().catch((error) => message.error(error instanceof Error ? error.message : "刷新失败"))}>
                            刷新
                        </Button>
                ...
                <Table rowKey="runId" columns={columns} dataSource={rows} loading={query.isLoading} pagination={{ pageSize: 20, showSizeChanger: true }} />
                <StartRunModal open={startOpen} loading={starting} token={token} onCancel={() => setStartOpen(false)} onStart={(templateId, inputs, productConcurrency, maxRetries) => void startRun(templateId, inputs, productConcurrency, maxRetries)} />
            </div>
        </main>
    );
}

function StartRunModal({ open, loading, token, onCancel, onStart }: { open: boolean; loading: boolean; token: string; onCancel: () => void; onStart: (templateId: string, inputs: Array<Record<string, unknown>>, productConcurrency?: number, maxRetries?: number) => void }) {
    const { message } = App.useApp();
    ...
    const templateQuery = useQuery({
        queryKey: ["pdd-workflow-templates", "start", token],
    ...
    return (
        <Modal title="启动电商工作流" open={open} onCancel={onCancel} width={820} destroyOnClose={false} footer={<Space><Button onClick={onCancel}>取消</Button><Button type="primary" icon={<Play className="size-4" />} loading={loading} onClick={submit}>启动</Button></Space>}>
            <div className="flex flex-col gap-4">
                ...
                <Card size="small" title="运行参数">
                    <div className="grid grid-cols-2 gap-3">
                        ...
                        <InputNumber className="!w-full" min={1} max={100} value={maxRetries ?? selectedTemplate?.spec.settings.maxRetries ?? 3} addonBefore="重试" onChange={(value) => setMaxRetries(Number(value || 1))} />
                    </div>
                </Card>
