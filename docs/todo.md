...
- 继续扩展 Local Workflow Executor：hybrid ecommerce 已先接入单条确认模板的 VPS PDD API backend；后续补远端事件更细粒度增量同步、`image_edit`、`video_generation`、复杂 loop/guardrail、多轮质检修复、自动素材匹配和更完整的失败恢复；继续不复用现有 PDD/VPS run 目录作为事实源。
