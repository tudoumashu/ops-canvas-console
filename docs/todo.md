...
- 继续扩展 Local Workflow Executor：hybrid ecommerce 已先接入单条确认模板的 VPS PDD API backend，并提供 `opsc ecommerce import-template` / `create-run` 的本地 headless CLI 路径和 `tools/hybrid_ecommerce_vps_smoke.py` smoke helper；真实 VPS smoke 已跑通导入、创建本地 run、远端 dispatch、状态同步和 5 个 canonical artifact/ref 回写。后续补 Web UI 发起同一 hybrid run 的浏览器回归、远端事件更细粒度增量同步、常驻本地 worker/watch mode 产品化、`image_edit`、`video_generation`、复杂 loop/guardrail、多轮质检修复、自动素材匹配和更完整的失败恢复；继续不复用现有 PDD/VPS run 目录作为事实源。
