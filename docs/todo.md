...
- 继续扩展 Local Workflow Executor：hybrid ecommerce 黄金路径已经具备 `opsc executor --watch` worker、Web UI local run 启动、远端阶段同步、canonical artifact/ref 回写、worker runtime/doctor 诊断、安装/自启动文档和可复用 smoke helper；Phase 14 已为已确认电商模板补充 local-first import/create-run、`material_lookup` 自动匹配本地 `anime_ip` 素材库、`image_edit`、内置 mockup 底版 fallback、项目相对打包和 `sync_local` marker，并已用真实浏览器、真实本地素材库、真实模型通道和隔离 project output 跑通 local-first Web smoke。后续优先补 `video_generation`、超出已确认模板的通用 `image_edit`/素材匹配配置、复杂 loop/guardrail、多轮质检修复、更完整失败恢复和用户真实长期 workspace 回归；继续不复用现有 PDD/VPS run 目录作为事实源。
