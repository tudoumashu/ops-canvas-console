...
本轮在用户本机部署使用路径上继续收口：本地电商 run 详情页已恢复为 VPS PDD 那种总体画布视图，`/workflows/ecommerce/run_<id>` 与 `/workflows/pdd/run_<id>` 对本地 `run_` ID 复用 `PDDRunCanvasPage` 并通过 `opsc serve` local PDD API 读取 canonical run/artifact。本地 run 现在默认进入商品流程画布而不是旧概览表格；当前 workspace 模板 `电商商品源图生成` 已改成三路 prompt 源图生成，并通过 executor `source_variants` package 模式输出到项目相对 `runs/<timestamp>/generated/<theme> - <character>`。最新修复把该模板图片节点切到低延迟 preset，并用真实输入 `{"character":"八重神子","theme":"《原神》","presentation":"feminine"}` 跑通 `run_01KSZSZ4WJ0V7C5XNWNG633VVD`：6 个节点全部 `success`、5 个 artifact ref、三张源图均在 120 秒内完成。模型配置主路径已经切到后端“云端渠道”，本机 SQLite `settings` 表已导入 VPS 已部署画布项目中的 `VPS ChatGPT2API` 和 `VPS Flow2API` 两个私有渠道；浏览器新会话默认使用云端渠道，但仍可在配置弹窗切回 local workspace profile/`secretRef` 本地直连。密钥和上游地址只存在本机 ignored DB，不写入 repo、浏览器持久化或默认输出。

Phase 14 已把已确认电商模板从 hybrid VPS-backed fallback 继续推进到 local-first 可执行路径：`opsc ecommerce import-template --local-executable` 会生成 `localEcommerce.backend=local_first` template，`opsc ecommerce create-run` 按 template metadata 自动选择 hybrid 或 local-first，executor 可执行该模板需要的本地 `material_lookup` 自动 `anime_ip` 素材匹配、`image_edit`、内置 mockup 底版 fallback、项目相对 `package` 和 `sync_local` marker。local workspace 仍是 canonical source；现有 PDD/VPS run 历史不迁移，VPS run dir 不作为事实源，浏览器不保存或发送 VPS admin credential，MCP 写面不扩大。Phase 14 已用隔离 workspace 跑通真实浏览器 + 真实本地素材库 + 真实 profile/channel `secretRef` 模型通道 + 真实 project output 的 local-first ecommerce Web smoke；后续重点转为真实长期个人 workspace 和安装/systemd 态 worker 回归。

## Completed Work

- 本轮确认旧“运行概览/节点状态”表格来自 stale 本地 Web build / local run 状态页路径；当前源码和已重启的本机 `next start` 已让本地 `run_` 进入 PDD 总体画布。
- 本轮把本地 `run_` 默认视图切到 VPS 风格商品流程画布，并让 local PDD 节点摘要优先显示输入 JSON、素材匹配、输出文件和项目输出数量。
- 本轮把当前 workspace 模板 `tpl_01KSZF9PKFRXFDDSVTZV8CTD2C` 改造成三路 `image_edit` 源图生成：日式、3D、韩式半写实三个 prompt 文件并行执行，第四个空 prompt 暂不启用；新增 `package_sources` 节点把三张图写到 `runs/<timestamp>/generated/<theme> - <character>`。
- 本轮 executor 的 local ecommerce `package` 动作新增 `source_variants` 模式；自动化覆盖三路 image_edit、项目相对输出、manifest、canonical artifact/ref、幂等边界和不触发 VPS run orchestration event。
- 本轮新增/使用 local PDD overview、product-detail、creative-canvas 映射，让本地 canonical run/artifact 可以喂给 VPS 风格 `PDDRunCanvasPage`，保留节点点击和 artifact 预览的数据边界。
- 本轮修复 local PDD “创作画布”保存态修复逻辑：creative-canvas API 会用当前 product detail/template DAG 生成 artifact 节点和边，再与旧保存画布合并；旧保存画布如果缺边或节点碰撞，会按真实媒体尺寸重新布局，避免刷新后继续显示无连线和重叠节点。
- 本轮修正 executor 图片请求参数：`quality` 归一化为上游支持的值，比例尺寸会按质量换算为像素尺寸，避免 `gpt-image-2` / ChatGPT2API 因 `2:3` 这类 ratio size 卡住或失败。
- 本轮把当前 workspace 模板/profile 的图片节点临时切到已验证可用的 ChatGPT2API `openai` 通道；Flow2API tunnel 可达但上游当前没有可用 token，先不把它作为当前模板的图片执行主路径。
- 本轮真实跑通当前模板 run `run_01KSZ6F3JPM3JBAHG0QEC6P662`：8 个节点全部 `success`，写回 7 个 artifact ref；`/workflows/ecommerce/run_...` 和 `/workflows/pdd/run_...` 均返回 200，Playwright 确认整体 DAG 画布存在、旧表格标记不存在。
- 本轮确认模型配置来源：浏览器“云端渠道”通过 `/api/settings` 读取公开模型列表和默认模型，真实 `apiKey/baseUrl` 由后端 `settings.private.channels` 根据模型名选择并代理；local workspace profile/`secretRef` 是另一条“本地直连”路径。
- 本轮已从 VPS 已部署画布项目导入 `VPS ChatGPT2API` 和 `VPS Flow2API` 两个云端渠道到本机 `data/infinite-canvas.db` 的 `settings` 表，保留公开 178 个模型与默认模型；临时导出文件已删除，本机 DB 已留 ignored 备份。
- 本轮前端默认模型模式从 `local` 改为 `remote`，新浏览器会话优先走后端云端渠道；用户仍可在配置弹窗切回本地直连 profile/`secretRef`。
- 本轮排查 `npm run local:prod` 自动断开原因是 3000 端口已有上一轮后台 `next start` 进程监听；已停止该监听进程并确认 3000 端口释放。
- 本轮修复本地 Web UI 日常使用卡顿入口：确认慢切页主因是 `next dev --webpack` 按需编译，新增 `web` scripts `local:prod` / `local:dev`，README 与安装文档同步本地生产模式启动方式。
- 本轮清理 Next dev overlay 已确认 Issues：AntD v6 deprecated API 改为 `Alert.title`、`Space.orientation`、`Modal.destroyOnHidden`、`Space.Compact` 和 `Drawer.size`；`rg` 已确认旧 API 使用点清空。
- 本轮已把当前本机 3000 端口从 `next dev` 切到 `next start`；生产模式主要路由 `curl` 抽测约 8-21ms。
...
- Phase 14 新增 `internal/localworkspace/local_ecommerce.go`、`opsc ecommerce import-template --local-executable` 和 local-first ecommerce run 创建路径；`CreateEcommerceRun` 会在 hybrid 和 local-first template 之间选择正确 backend，保留 hybrid fallback。
- Phase 14 扩展 executor：`material_lookup` 支持自动扫描本地 `anime_ip` 素材库 fixture/配置路径并写 canonical artifact，路径来自 template metadata `materialLibraryPath` 或 executor runtime env `OPSC_LOCAL_ECOMMERCE_MATERIAL_LIBRARY`，代码不硬编码本机绝对路径；`image_edit` 复用 workspace profile `secretRef` OpenAI-compatible multipart 调用，并按 template/run metadata 选择 channel；`package`/`sync_local` 可在 project capability/path guard 下写项目相对输出和 marker；内置 mockup base fallback 避免浏览器必须预先 materialize 服务器固定素材。
- Phase 14 import/create-run 已支持把默认 `projectId` 写入 local ecommerce template settings，Web UI 从该 template 启动 run 时无需再次手工传项目即可进入 project-aware executor runtime。
- Phase 14 真实浏览器 smoke 已通过：Web UI 从 local-first ecommerce template 发起 run，`opsc executor --watch` 执行到 `success`，本地素材库匹配 `药屋少女的呢喃 / 猫猫`，写入 7 个 canonical artifact 和 5 个项目相对输出文件，artifact preview 可打开，浏览器持久化未发现模型/VPS secret、token、launch secret 或 secret file content，事件中没有 remote/hybrid dispatch。
- Phase 14 Web local template adapter 会为本地电商模板写入 `metadata.localEcommerce`，保存已导入 hybrid template 时继续保留 `metadata/settings.hybridEcommerce`，避免把 VPS-backed 模板误改成本地执行模板。
...
- executor 仍是 MVP：覆盖固定本地素材、文本生成、图片生成、条件分支、受 project guard 约束的脚本节点，以及单条已确认电商模板的 hybrid VPS PDD backend；Phase 14 增加该电商模板的 local-first 路径，覆盖自动 `anime_ip` 素材匹配、`image_edit`、mockup base fallback、project package/sync action。真实 VPS smoke 已证明导入/dispatch/sync/artifact ref 黄金路径可行，Phase 12 已补 `opsc executor --watch` 和 Web local hybrid run 路径。Phase 13 已用隔离长期回归 workspace 和非临时 browser profile 跑通 local workspace UI smoke 与 hybrid Web + watch fake VPS smoke；当前机器未发现用户真实长期 workspace，因此历史个人 workspace 数据仍未实测。`video_generation`、复杂 loop/guardrail、完整模板重试策略、通用素材匹配配置、通用 image_edit 编排、专用文章/视频/更多电商 project adapter、真实 worker 安装/自启动和真实 PDD/VPS run 历史迁移仍待后续阶段。
...
- 下一阶段：优先用用户真实长期 workspace 和安装/systemd 态 worker 重复跑 Phase 14 local-first ecommerce Web run；然后补 `video_generation`、更完整 retry/recovery 和安装分发。不要先扩大 MCP 写面或迁移旧 VPS run 历史。
- executor 后续要补通用化的 `image_edit` 编排、`video_generation`、复杂 loop/guardrail、模板级失败重试语义和更完整的失败恢复策略；继续复用 workspace core、profile `secretRef` 和 canonical artifact/ref 写回路径。
...
- passed：本轮已用真实 ChatGPT2API 链路执行当前本地电商模板 run `run_01KSZ6F3JPM3JBAHG0QEC6P662`，结果为 `success`，8 个 node state 全部 `success`，7 个 run artifact ref。
- passed：本轮 local PDD “创作画布”修复已运行 Docker `golang:1.25-alpine` `gofmt`、`GOPROXY=https://goproxy.cn,direct go test ./internal/localworkspace ./cmd/opsc` 和 `go build ./cmd/opsc`；重启 `opsc serve` 后，当前 run 的 creative-canvas API 返回 7 个节点、8 条边、0 个重叠，Playwright 打开页面后确认渲染 8 条 connection path。
- passed：本轮已运行 Docker `golang:1.25-alpine` `gofmt`、`GOPROXY=https://goproxy.cn,direct go test ./internal/localworkspace ./cmd/opsc` 和 `go build ./cmd/opsc`；已运行 `cd web && npx tsc --noEmit`、`API_BASE_URL=http://127.0.0.1:8080 npm run build`。
- passed runtime：本轮已重启本机 Go API、`opsc serve`、ChatGPT2API/Flow2API local tunnel 和 Web `next start`；`curl` 抽测 `/workflows/ecommerce/run_01KSZ6F3JPM3JBAHG0QEC6P662` 与 `/workflows/pdd/run_01KSZ6F3JPM3JBAHG0QEC6P662` 返回 200。
- passed browser smoke：本轮已用 Playwright 建立 `opsc serve` browser session 并访问本地电商 run 页，确认页面显示整体 DAG 画布且旧“运行概览/节点状态”表格标记不存在；截图证据在本机临时目录。
- passed：本机后端 `/api/settings` 已返回 178 个公开模型，默认文本/图片/视频模型分别为 `gpt-5.5`、`gpt-image-2`、`veo_3_1_t2v_fast_portrait`；本机 DB 中 2 个私有渠道均已写入密钥字段但未打印明文。
- passed：3000 端口上的旧 `next start` 监听进程已停止，`ss -ltnp 'sport = :3000'` 不再显示监听者。
- passed：本轮模型渠道默认云端修改后已运行 `cd web && npx tsc --noEmit`、`git diff --check`、Wiki `lint_wiki.sh`、`reindex_qmd.sh llm-wiki` 和 `qmd embed`。
- passed：本轮本地 Web 收口已运行 `cd web && npx tsc --noEmit`、`cd web && npm run build`、`git diff --check`；生产构建过程中 `next-env.d.ts` 生成态变化已恢复，不纳入提交。
- passed runtime：本轮已启动 `next start -H 0.0.0.0 -p 3000`，`curl --noproxy '*'` 抽测主要静态路由返回 200 且无 dev 编译等待。
...
- passed：Phase 14 已用 Docker `golang:1.25-alpine` 执行 `/usr/local/go/bin/gofmt -w internal/localworkspace/executor.go internal/localworkspace/local_ecommerce.go internal/localworkspace/hybrid_ecommerce.go internal/localworkspace/executor_test.go cmd/opsc/main.go cmd/opsc/main_test.go`。
- passed：Phase 14 已运行 `GOPROXY=https://goproxy.cn,direct /usr/local/go/bin/go test ./internal/localworkspace ./cmd/opsc`，覆盖 local-executable ecommerce template import、local ecommerce create-run、自动素材匹配、builtin mockup base fallback、image_edit fake provider、package/sync project output、idempotent rerun 和 secret/rootPath 脱敏。
- passed：Phase 14 已运行 `cd web && npx tsc --noEmit`。
- passed browser smoke：Phase 14 已用隔离 workspace、真实浏览器、真实本地 `anime_ip` 素材库、真实 profile/channel `secretRef` 模型通道和真实 project output 目录跑通 local-first ecommerce Web run；Web UI 创建 run 后 `opsc executor --watch` 推进到 `success`，7 个 canonical artifact、5 个 project output 文件、artifact modal 预览和浏览器持久化脱敏检查均通过。用户真实长期 workspace 和安装/systemd 态 worker 仍需后续重复验收。
