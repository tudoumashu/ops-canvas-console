...
- executor 仍是 MVP：只覆盖固定本地素材、文本生成、图片生成、条件分支、受 project guard 约束的脚本节点，以及单条已确认电商模板的 hybrid VPS PDD backend；真实 VPS smoke 已证明导入/dispatch/sync/artifact ref 黄金路径可行，Phase 12 已补 `opsc executor --watch` 和 Web local hybrid run 路径。Phase 13 已用隔离长期回归 workspace 和非临时 browser profile 跑通 local workspace UI smoke 与 hybrid Web + watch fake VPS smoke；当前机器未发现用户真实长期 workspace，因此历史个人 workspace 数据仍未实测。`image_edit`、`video_generation`、复杂 loop/guardrail、完整模板重试策略、自动素材匹配、专用文章/视频/电商 project adapter、真实 worker 安装/自启动和真实 PDD/VPS run 历史迁移仍待后续阶段。
