...
- Phase 14 扩展 executor：`material_lookup` 支持自动扫描本地 `anime_ip` 素材库 fixture/默认路径并写 canonical artifact；`image_edit` 复用 workspace profile `secretRef` OpenAI-compatible multipart 调用；`package`/`sync_local` 可在 project capability/path guard 下写项目相对输出和 marker；内置 mockup base fallback 避免浏览器必须预先 materialize 服务器固定素材。
...
- 下一阶段：优先用真实浏览器、真实长期 workspace、真实 `anime_ip` 素材库和低成本 image-edit provider 跑 Phase 14 local-first ecommerce Web run；然后补 `video_generation`、更完整 retry/recovery 和安装分发。不要先扩大 MCP 写面或迁移旧 VPS run 历史。
...
- not run：Phase 14 尚未用真实浏览器、真实长期 workspace、真实 image-edit provider 和真实 project output 目录跑 local-first ecommerce Web run；已写入 `docs/pending-test.md` 与 `docs/manual-test-report-phase14.md`。
