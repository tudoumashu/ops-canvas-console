...
## `npm run local:prod` 会长期占用 3000

`web` 下的 `npm run local:prod` 会先 `next build`，再用 `next start -H 0.0.0.0 -p 3000` 常驻运行。它不会像一次性脚本那样自动退出；如果另一个终端或后台进程已经占用 3000，新的启动会在 build 成功后报 `EADDRINUSE`。排查时先用 `ss -ltnp 'sport = :3000'` 找监听进程，确认不是用户正在使用的浏览器服务后再停止它。
...
Flow2API 端口或 SSH tunnel 可达不等于上游有可用生成 token。当前本地验证中 Flow2API provider 返回过“无可用 token/账号不可用”类错误；在 token 刷新前，已确认电商模板应优先走 ChatGPT2API / `gpt-image-2` 通道，避免误判为 executor 或画布页面 bug。

本地 Web UI 创建 local run 只会写入 `run.waiting_for_executor` / `pending` 状态，不会自动执行节点。用户如果只打开 Web UI 而没有运行 `opsc executor --watch`，电商 run 会看起来“没有反应”。调试单个 run 时优先用 `opsc --workspace <workspace> --json executor --run <run_id>`；长期使用再启动 watch worker。不要在有大量旧 pending run 时直接启动 watch worker，除非确认这些 run 可以消耗模型额度。

ChatGPT2API / `gpt-image-2` 图片编辑请求对 prompt 长度、`quality` 和尺寸很敏感。当前已确认电商模板的本地执行 preset 使用 `quality=low`、`size=1024x1024` 和紧凑 prompt，真实 smoke 中三张源图均在 120 秒内完成；如果把模板改回高质量纵向长 prompt，单张图片可能接近或超过 120 秒，需要先做低成本 probe 再恢复到常用模板。

本地 `run_` 详情页如果仍显示“运行概览/节点状态”表格而不是 PDD 总体画布，优先检查 3000 上是否还跑着旧 `next start` build。前端路由源码已把 `/workflows/ecommerce/run_<id>` 和 `/workflows/pdd/run_<id>` 接到 `PDDRunCanvasPage`；改完后需要重新 `next build` 并重启 `next start`。
...
本地 `opsc serve` 的 creative-canvas 端点也不能直接返回旧保存画布。旧保存文件可能长期保留 `edges: []`，并且节点位置是按旧 fallback 尺寸计算的；前端按真实媒体尺寸渲染后会变成无连线、节点重叠。返回前必须先用当前 product detail/template DAG 规范化：补齐可见 artifact 节点和连接边，读取 canonical artifact 的真实尺寸，并在缺边或碰撞时重新留间距。
