...
- 电商工作流私有模板与本地 run：连接本地工作区时，模板列表和模板编辑器通过 `local-workflow-templates.ts` 调 `opsc serve` 写入 `templates/<tpl_id>/template.json`；`material_lookup` 固定素材下拉读取当前 workspace 的 `我的素材` 图片列表，不再请求服务器 admin asset 接口；运行本地模板会创建 `runs/<run_id>/run.json`、节点状态和 `run.waiting_for_executor` 事件，并进入本地 run 状态页。固定 `material_lookup` 图片素材会从 `assets/<asset_id>/files/original` 复制成全局 `artifacts/<art_id>/files/original`，run 内只保存 artifact ref；如果 Web 创建 run 时已预写固定素材节点，`opsc executor` 会跳过已成功节点。Phase 11 hybrid ecommerce 模板通过 `metadata.hybridEcommerce` 记录远端 template id/profile/channel，`opsc ecommerce create-run` 可不经浏览器创建 pending local run，executor 调 VPS PDD API 执行并把状态、events 和 artifact ref 同步回本地 workspace；远端 run dir 不作为 canonical data。自动匹配 local asset、`image_edit`/`video_generation` 和更完整的专用 adapter 仍待下一阶段处理。
