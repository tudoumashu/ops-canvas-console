...
- 验证：已用 Docker `golang:1.25-alpine` 执行 `gofmt`；已运行 `go test ./internal/localworkspace ./cmd/opsc`；已运行 `cd web && npx tsc --noEmit`；已运行 `python3 -m py_compile tools/hybrid_ecommerce_browser_smoke.py tools/local_workspace_browser_smoke.py tools/hybrid_ecommerce_vps_smoke.py`；已用临时 workspace、`opsc serve`、Next dev server、Chrome 和 fake VPS 执行 `tools/hybrid_ecommerce_browser_smoke.py` 通过；又用同一条已确认 VPS API/template 黄金路径执行真实 Web UI + `opsc executor --watch` smoke，通过模板编辑页“运行模板”发起 local run，远端 run 同步到本地 `success`，写回 5 个 canonical artifact/ref，run 状态页 artifact modal 可预览，浏览器持久化未发现 VPS token、bearer token、launch secret 或 configured secret env name；中央 Wiki 已运行 `lint_wiki.sh`、`reindex_qmd.sh llm-wiki` 和 `qmd embed`。
...
- 风险：真实长期个人 workspace、非临时浏览器 profile 和 worker 安装/自启动后的恢复体验仍需人工回归；`image_edit`/`video_generation` 和更完整 retry/recovery 仍在后续阶段。
