...
- 已确认电商模板可导入为 `localEcommerce.backend=local_first` 的本地执行模板；素材库路径由导入 metadata 或 `OPSC_LOCAL_ECOMMERCE_MATERIAL_LIBRARY` 提供，executor 在本机完成自动 `anime_ip` 素材匹配、模板内 `image_edit`、项目相对 `package` 和 `sync_local`。
...
- local run 已接入 `opsc executor` MVP，可执行固定本地素材、`text_generation`、`image_generation`、`condition` 和受 project capability/path guard 约束的本地 `script` 最小节点集；hybrid ecommerce 已先接通单条已确认 PDD 模板的 VPS API backend，可把远端执行结果同步为本地 canonical artifact/ref。已确认电商模板的 local-first 路径进一步支持自动本地素材匹配、模板内 `image_edit`、本地项目相对 `package/sync_local`；`video_generation`、超出已确认模板的通用 `image_edit`/素材匹配配置、专用文章/视频 adapter、完整失败策略、通用远端模板平台和 PDD/VPS 历史 run 迁移仍未完成。
