...
Phase 10 已继续收口 Local Workflow Executor MVP：唯一正式入口仍是 `opsc executor --workspace <path>`，它领取带 `run.waiting_for_executor` 的 local run，执行固定本地素材 `material_lookup`、`text_generation`、`image_generation`、最小 `input/text_static`、`condition` 和受 project capability/path guard 约束的本地项目 `script` 节点，复用 workspace profile `secretRef` 调用 OpenAI-compatible provider，并把 node state、append-only events、canonical artifact、run artifact ref 和 project output mapping 写回同一 workspace 或明确授权的项目相对路径。Phase 11 新增单条已确认电商模板的 hybrid VPS backend：通过 `opsc ecommerce import-template` 重建远端 PDD template 的 local canonical copy，`opsc executor` 用 profile/channel `secretRef` 调 VPS API 创建远端 run、轮询状态并下载关键 artifact 回写本地 workspace；local run 仍是事实源，VPS run dir 和历史 PDD/VPS run 不迁移。运行时自动 `material_lookup` 匹配、专用文章/视频/电商 project adapter、`image_edit`/`video_generation`、复杂 loop/guardrail 和完整重试策略仍待后续阶段。
...
7. Web UI 改为通过 `opsc serve` 访问本地 workspace。已完成本地工作区连接入口、`我的素材`、`我的提示词`、画布项目库、画布项目内媒体文件、画布导入/导出 zip 媒体文件、工作台 text/image/video 生成记录、AI profile/proxy、电商工作流私有模板 CRUD、模板 `material_lookup` 固定素材本地选择、工作流入口自定义文件夹、本地 run 列表/状态页、run event/node/artifact ref 写入 API 和本地模板创建 run 草稿；AI profile 读取兼容完整 `secretRef.name` 和脱敏 `secretRef.reference`；图片/视频工作台结果保存成功后会从 workspace 文件端点回显；素材包导入不再写浏览器媒体库作为事实源，画布 zip 导入结束后清理临时 import blob；本地模板启动 run 时可写入 `profileId/projectId`，供 executor 使用 project capability/path guard；真实 PDD/VPS executor、PDD run 数据迁移和运行时 `material_lookup` local asset id 自动解析仍待迁移。
