...
Phase 10 已继续收口 Local Workflow Executor MVP：唯一正式入口仍是 `opsc executor --workspace <path>`，它领取带 `run.waiting_for_executor` 的 local run，执行固定本地素材 `material_lookup`、`text_generation`、`image_generation`、最小 `input/text_static`、`condition` 和受 project capability/path guard 约束的本地项目 `script` 节点，复用 workspace profile `secretRef` 调用 OpenAI-compatible provider，并把 node state、append-only events、canonical artifact、run artifact ref 和 project output mapping 写回同一 workspace 或明确授权的项目相对路径。当前仍不迁移现有 PDD VPS run；PDD run 数据仍未迁到 local workspace；运行时自动 `material_lookup` 匹配、专用文章/视频/电商 project adapter、`image_edit`/`video_generation`、复杂 loop/guardrail 和完整重试策略仍待后续阶段。
