...
- Hybrid ecommerce 只接一个已确认模板路径：远端 VPS API 是执行后端，不是 local workspace 的事实源；浏览器不得保存或发送 VPS admin credential。
...
7. Web UI 改为通过 `opsc serve` 访问本地 workspace。已完成本地工作区连接入口、`我的素材`、`我的提示词`、画布项目库、画布项目内媒体文件、画布导入/导出 zip 媒体文件、工作台 text/image/video 生成记录、AI profile/proxy、电商工作流私有模板 CRUD、模板 `material_lookup` 固定素材本地选择、工作流入口自定义文件夹、本地 run 列表/状态页、run event/node/artifact ref 写入 API 和本地模板创建 run 草稿；AI profile 读取兼容完整 `secretRef.name` 和脱敏 `secretRef.reference`；图片/视频工作台结果保存成功后会从 workspace 文件端点回显；素材包导入不再写浏览器媒体库作为事实源，画布 zip 导入结束后清理临时 import blob；本地模板启动 run 时可写入 `profileId/projectId`，供 executor 使用 project capability/path guard；单条已确认电商模板的 hybrid VPS executor 路径已接通，通用 PDD/VPS executor、本地 Web UI 真实发起 hybrid run、PDD run 历史迁移和运行时 `material_lookup` local asset id 自动解析仍待迁移。
...
- 不在 MCP 中实现 canonical object 写入工具、模型调用工具或 local executor tool；本地执行入口保持为 `opsc executor` CLI。
