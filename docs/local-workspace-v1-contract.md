...
- stabilization status：Phase 8 只做 hardening / verification / docs-sync，不新增 local executor、不迁移 PDD/VPS run、不做 Full GC、不扩大 MCP 写能力、不新增 canonical object 类型。已补充 `opsc serve` runtime/session/auth/redaction 回归、AI proxy `secretRef` 与浏览器 header 隔离回归、本地模板草稿 run 到 canonical artifact ref 的 happy path 回归，并继续以 `go test ./cmd/opsc` 覆盖 MCP stdio wrapper smoke。
