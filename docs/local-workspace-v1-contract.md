...
- 当前 Web UI 已迁移素材、提示词、画布项目、画布项目内媒体文件、工作台 text/image/video 生成记录、AI 本地直连 profile/proxy、电商工作流私有模板 CRUD、模板 `material_lookup` 固定素材本地选择、工作流入口自定义文件夹、本地 run 列表/状态页/artifact 预览、本地模板创建 run 草稿和固定素材 artifact ref。Phase 10 `opsc executor` 已能执行固定本地素材、文本生成、图片生成、条件分支和受 project capability/path guard 约束的本地脚本节点；真实 PDD/VPS executor、PDD/VPS run 数据迁移、运行时 `material_lookup` 自动匹配本地素材、`image_edit`/`video_generation` 和专用文章/视频/电商 project adapter 仍未完成；现有 PDD/VPS run 仍不是 local workspace canonical data。
...
- 真实 PDD/VPS executor、PDD/VPS run 数据迁移、运行时 `material_lookup` 自动匹配本地素材、专用文章/视频/电商 project adapter 和 `image_edit`/`video_generation`/复杂 loop/guardrail 仍需下一阶段细化。
