...
- hybrid ecommerce status：Phase 11 新增单条已确认电商模板的 hybrid VPS backend 路径。`opsc ecommerce import-template` 通过本地 profile/channel 的 `secretRef` 读取 VPS PDD template，重建为 local workspace `templates/<tpl_id>/template.json`；local run 仍以 `runs/<run_id>/run.json`、node state、events 和 artifact refs 为 canonical。`opsc executor` 遇到带 `metadata.hybridEcommerce.backend=vps_pdd` 的模板时，只通过受控 PDD API 创建远端 run、轮询 overview/product-detail、下载确认 artifact 文件并写回本地 canonical `artifacts/`；远端 run dir 和既有 PDD/VPS run 历史都不是本地事实源。浏览器仍只连接 `opsc serve`，不得保存或发送 VPS admin token、cookie 或明文 secret；MCP 写面不变。
...
- Hybrid ecommerce VPS 后端只能由 `opsc` 本地进程使用 profile/channel `secretRef` 调用；浏览器通过 `opsc serve` 创建/查看 local run，不直接调用 VPS admin API，不保存或发送 VPS admin token/cookie/secret。同步回 workspace 的数据仅限远端 run id、状态摘要、append-only events、node state 和已下载的 canonical artifact/ref；VPS run dir 不进入 local canonical metadata。
